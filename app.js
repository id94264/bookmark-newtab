...
const defaultSettings = {
  ...
  hiddenFolders: [],
  viewLayout: 'grid'          // 默认布局：grid（网格图标）或 list（列表）
};
...
async function loadSettings() {
  ...
  if (res.settings) {
    currentSettings = {
      ...
      viewLayout: res.settings.viewLayout || 'grid'
    };
  }
}
...
async function renderCurrentFolder() {
  ...
  // 核心改动：根据设置清除旧布局，赋予对应的新布局类名 (grid-view 或 list-view)
  container.className = currentSettings.viewLayout === 'list' ? 'list-view' : 'grid-view';
  ...
  if (container.children.length === 0) {
    container.innerHTML = '<div style="text-align: center; color: #888; padding: 40px; grid-column: 1/-1;">该文件夹下没有可显示的内容</div>';
  }
}
...
async function initSettingsModal() {
  ...
  btn.addEventListener('click', async () => {
    ...
    // 初始化当前选择样式
    layoutSelect.value = currentSettings.viewLayout;
  ...
  saveBtn.addEventListener('click', async () => {
    ...
    currentSettings.viewLayout = layoutSelect.value; // 保存布局样式
